...
class var_map():
    def __init__(self):
        self.pub = rospy.Publisher('VAR_array', MarkerArray, queue_size=10)
        self.aux = 0
        self.aux2 = 0
        self.first = True
        self.update_rate = 20
    ...
    def map_subscriber(self,msg):
        ...
        self.data = msg.data
        self.robot_position = np.zeros((msg.info.width,msg.info.height))
...
######## MARPIRlab
    ...
    def var_map(self,x,y):
        ...
        if self.aux2 >=5 or self.first == True:
            for i in xrange(-5,6):
                for j in xrange(-5,6):
        ...
        for x in xrange(self.resized.shape[1]):
            for y in xrange(self.resized.shape[0]):
                ...
                if self.resized[y][x] == 0:
                    ...
                    self.marker.points = [self.point_msg]
